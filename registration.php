...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!empty($username) && !empty($user_password) && !empty($user_email)){
        ...
        $p_len = strlen($user_password);
        if ($u_len < 6){
            $message = "<p class='bg-danger'>Username must be at least 6 characters;</p>"; 
        }else if($p_len < 8){
            $message = "<p class='bg-danger'>Password must be at least 8 characters;</p>"; 
        }else if (mysqli_num_rows($res_u) > 0) {
            $message =  "<p class='bg-danger'>Sorry... username already taken;</p>"; 	
        }else if(mysqli_num_rows($res_e) > 0){
            $message = "<p class='bg-danger'>This email is already registered <a href='index.php'>Log In</a></p>"; 		
        }else{
